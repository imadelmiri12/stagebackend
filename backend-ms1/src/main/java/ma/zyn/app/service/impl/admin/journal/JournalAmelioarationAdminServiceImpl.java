...
import java.util.List;
@Service
public class JournalAmelioarationAdminServiceImpl implements JournalAmelioarationAdminService {
    ...
    public void findOrSaveAssociatedObject(JournalAmelioaration t){
        if( t != null) {
            t.setSystemeManagement(systemeManagementService.findOrSave(t.getSystemeManagement()));
            t.setCategorieEcrate(categorieEcrateService.findOrSave(t.getCategorieEcrate()));
            t.setOrigineEcrate(origineEcrateService.findOrSave(t.getOrigineEcrate()));
            t.setTypeAction(typeActionService.findOrSave(t.getTypeAction()));
            t.setCollaborator(collaboratorService.findOrSave(t.getCollaborator()));
            t.setCritereEvaluation(critereEvaluationService.findOrSave(t.getCritereEvaluation()));
            t.setResultatEvaluation(resultatEvaluationService.findOrSave(t.getResultatEvaluation()));
        }
    }
    ...
    @Autowired
    private TypeActionAdminService typeActionService ;
    @Autowired
    private CollaboratorAdminService collaboratorService ;
    @Autowired
    private ResultatEvaluationAdminService resultatEvaluationService ;
    @Autowired
    private SystemeManagementAdminService systemeManagementService ;
    @Autowired
    private CritereEvaluationAdminService critereEvaluationService ;
    @Autowired
    private CategorieEcrateAdminService categorieEcrateService ;
    @Autowired
    private OrigineEcrateAdminService origineEcrateService ;

    public JournalAmelioarationAdminServiceImpl(JournalAmelioarationDao dao) {
        this.dao = dao;
    }
